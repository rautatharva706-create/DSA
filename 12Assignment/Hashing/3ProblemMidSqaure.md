# Assignment No: 12 Problem: 3

## Title: Simulation of Employee Database Using Hash Table with Mid-Square Hashing and Linear Probing

### Theory

A hash table is a data structure that stores key–value pairs and provides efficient insertion, searching, and retrieval.  
The mid-square method squares the key and extracts the middle digits to compute the hash index.  
Linear probing is used to resolve collisions by searching sequentially for the next empty slot.

### Algorithm

#### Algorithm: Mid-Square Hashing
1. Start  
2. Square the key  
3. Convert square to string  
4. Find the middle position  
5. Extract two middle digits  
6. Apply modulo with table size  
7. Return index  
8. Stop  

#### Algorithm: Insert Employee Record
1. Compute index using mid-square hash  
2. If slot empty → insert record  
3. Else perform linear probing  
4. Insert into first free slot  
5. Stop  

#### Algorithm: Search Employee Record
1. Compute index using mid-square hash  
2. If ID matches at index → display record  
3. Else apply linear probing  
4. If full cycle completed → not found  
5. Stop  

#### Algorithm: Display Hash Table
1. For each index in table:  
   - If occupied → print details  
   - Else → print “Empty”  
2. Stop  
...
4. Exit
Enter your choice: 1
Enter Employee ID: 156
Enter Employee Name: Miss.Sharma
...
[0] Empty
[1] Empty
[2] 101 | Mr.Tiwari | Computer
[3] 156 | Miss.Sharma | Mechanical
[4] Empty
[5] Empty
[6] Empty
[7] Empty
